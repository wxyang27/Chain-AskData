...
`answer/` 不再负责主链路编排。主链路已经收敛到 `pipeline/`。

## 编排层

| 模块 | 职责 |
|---|---|
| `pipeline/objects.py` | PipelineRunResult、PipelineTrace、PipelineStageLog |
| `pipeline/pipeline.py` | AskDataPipeline 主编排，每个 `_stage_*` 对应一个可观测阶段 |

面试讲述时可以把 `pipeline/` 作为项目骨架核心：它回答“一个问题进来之后系统到底经历了哪些步骤”。
...
| `schema_index/` | 加载离线生成的字段/表/关系索引 JSON |
...
| 模块 | 职责 |
|---|---|
| `intent_router/` | 判断 nl2sql / schema_explain / caliber_explain / unknown |
| `semantic/` | 轻量业务语义契约，归一高风险指标、维度、过滤条件 |
| `query_planner/` | 生成 QueryPlan，并接入 QueryPlanCoT |
...
| 模块 | 职责 |
|---|---|
| `sql_generator/` | 确定性模板 SQL 生成，作为稳定 fallback |
| `llm/sql_generator.py` | LLM SQL 生成，输入 QueryPlanCoT + SchemaGraph |
| `llm/sql_safety_gate.py` | LLM SQL 采纳门禁，检查表字段、口径、日期、MaxCompute 语法 |
| `llm/sql_repairer.py` | 常见 SQL 静态修复 |
| `sql_validator/` | 基础 SQL 校验 |
...
| `llm/query_plan_cot_generator.py` | QueryPlanCoT 生成与修复 |
| `llm/query_plan_cot_validator.py` | QueryPlanCoT 本地约束校验 |
| `model_clients/` | Embedding / Rerank 抽象接口，预留 DashScope 切换 |
| `core/config.py` | 环境变量配置 |

`model_clients/` 当前主要是接口抽象，用于说明后续可以把本地 HashEmbedding / LightweightReranker 替换成 DashScope `text-embedding-v4` 和 `qwen-rerank`。
...
3. `pipeline/pipeline.py`：看完整主链路。
4. `pipeline/objects.py`：看 trace 和 run result。
5. `knowledge_indexer/service.py`：看 RAG 检索入口。
6. `schema_retrieval/askdata_style_retriever.py`：看 SchemaGraph 如何生成。
7. `query_planner/planner.py`：看 QueryPlan / CoT 如何生成。
8. `llm/sql_safety_gate.py`：看 SQL 为什么能被采纳或拒绝。
